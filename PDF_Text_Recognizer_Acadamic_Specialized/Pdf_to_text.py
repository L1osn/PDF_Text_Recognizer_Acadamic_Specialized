...
from citation_analyzer import CitationAnalyzer, AnalysisConfig, RefEntry, parse_bibliography_ids
...
class AcademicPDFRecognizer:
    ...
    def __init__(self, pdf_path: str, analysis_preset: str = "balanced"):
        ...
        # Citation Analyzer with configurable preset
        if analysis_preset == "strict":
            config = AnalysisConfig.strict()
        elif analysis_preset == "recall":
            config = AnalysisConfig.recall()
        else:
            config = AnalysisConfig.balanced()
    ...
    def _validate_superscripts(self):
        """
        Stage 2: Validate and analyze superscripts through CitationAnalyzer.
        Called after extraction to perform semantic analysis.
        Tokens now come from LayoutAnalyzer with line-based aggregation.
        """
        self.citation_analyzer.reset()
        
        # Try to extract bibliography IDs for constraint
        bib_ids = self._extract_bibliography_ids()
        if bib_ids:
            self.citation_analyzer.set_bibliography_ids(bib_ids)
        
        # DEBUG: Verify superscripts are being detected
        print(f"SUP TOKENS: {len(self.superscripts)}, sample: {self.superscripts[:10]}")
        
        # Process each superscript token (already merged by LayoutAnalyzer)
        for sup in self.superscripts:
            self.citation_analyzer.add_superscript(
                raw_text=sup.get('text', ''),
                page=sup.get('page', 0),
                bbox=sup.get('bbox', (0, 0, 0, 0)),
                anchor_text=sup.get('anchor', ''),
                page_height=sup.get('page_height', 792.0),
                line_num=sup.get('_line_num', 0)
            )
    
    def _extract_bibliography_ids(self) -> set:
        """Extract valid reference IDs from bibliography section"""
        ids = set()
        
        # Find References section in extracted text
        if not self.extracted_text:
            return ids
        
        # Common headers for references section
        headers = ['References', 'Bibliography', 'Literature Cited', 'Works Cited']
        ...
        for header in headers:
            ...
            match = re.search(rf'\n\s*{header}\s*\n', text, re.IGNORECASE)
            if match:
                bib_text = text[match.end():]
                ids = parse_bibliography_ids(bib_text)
                if ids:
                    break
        
        return ids
    
    def get_citation_entries(self) -> List[RefEntry]:
        """Get validated citation entries (for UI display)"""
        return self.citation_analyzer.get_sorted_entries()
    
    def get_citation_stats(self) -> Dict:
        """Get citation analysis statistics"""
        return self.citation_analyzer.get_stats()
    
    def set_analysis_preset(self, preset: str):
        """Change analysis preset (strict/balanced/recall)"""
        if preset == "strict":
            self.citation_analyzer.config = AnalysisConfig.strict()
        elif preset == "recall":
            self.citation_analyzer.config = AnalysisConfig.recall()
        else:
            self.citation_analyzer.config = AnalysisConfig.balanced()
